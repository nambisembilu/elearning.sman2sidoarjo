...
import '../../../controllers/guru/nilai_tugas/nilai_tugas_riverpod.dart';
...
class NilaiTugasScreen extends ConsumerStatefulWidget {
  const NilaiTugasScreen({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _NilaiTugasScreenState();
}

class _NilaiTugasScreenState extends ConsumerState<NilaiTugasScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await notifier.resetAndFetch(page: 1); // ini akan isi lastListNilaiTugas
  ...
  Future<void> _initFilters() async {
    try {
      ...
      if (notifier.lastListNilaiTugas.isEmpty) {
        // Kalau kosong, berarti user belum pernah fetch sama sekali
        print("lastListNilaiTugas kosong, tidak ada data filter terakhir.");
        // setState(() => _isInitializing = false);
        return;
      }

      // 🔹 1. Ambil filter terakhir
      final last = notifier.lastListNilaiTugas.first;
      selectedSemesterId = last.semesterId;
      selectedKmpId = last.kelasMapelId;
